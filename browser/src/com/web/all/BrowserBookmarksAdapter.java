...
class BrowserBookmarksAdapter extends BaseAdapter {
    ...
    public BrowserBookmarksAdapter(BrowserBookmarksPage b, String curPage,
        CharSequence curTitle, CharSequence bookmarkPattern, boolean createShortcut, int type) {
        mNeedsOffset = !(createShortcut || type == BrowserBookmarksPage.MOSTVISIT_BOOKMARK);
        mType = type;
        ...
        if (type == BrowserBookmarksPage.MOSTVISIT_BOOKMARK) {
            whereClause = Browser.BookmarkColumns.VISITS + " != 0";
        } else {
            whereClause = Browser.BookmarkColumns.BOOKMARK + " != 0";
            if (mType == BrowserBookmarksPage.SELF_BOOKMARK) {
              whereClause += " and " + Browser.BookmarkColumns.URL +
                " like '%" + bookmarkPattern +"%'";
            }
        }
    ...
    public Bitmap getTouchIcon(int position) {
        return getBitmap(HISTORY_PROJECTION_TOUCH_ICON_INDEX, position);
    }
    ...
    private void bind(BookmarkItem b, int position) {
        ...
        if (title.length() > MAX_TEXTVIEW_LEN) {
            title = title.substring(0, MAX_TEXTVIEW_LEN);
        }
        b.setName(title);
        String url = mCursor.getString(Browser.HISTORY_PROJECTION_URL_INDEX);
